...
import FormSubscription from '../components/Subscription/SMailchimp.jsx'
...
const Column= ({
    ...
    <ColumnDiv className={classNames(`${prefixCls}-column`, className)} style={style}>
      {(title || icon) && (
        <h2>
          {icon && <SpanIcon className={`${prefixCls}-column-icon`}>{icon}</SpanIcon>}
          {title}
        </h2>
      )}
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            ...
            >
              {item.icon && (
                <ItemIconSpan className={`${prefixCls}-item-icon`}>{item.icon}</ItemIconSpan>
              )}
